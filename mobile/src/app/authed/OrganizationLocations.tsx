...
const SPACING_FOR_CARD_INSET = width * 0.1 - CARD_SPACING;

type PropsOrg = NativeStackScreenProps<
  AuthedParamList,
...
export const OrganizationLocations = ({ route }: PropsOrg) => {
  ...
  const { data } = useGetOrgLocationsQuery(
    client,
    { id },
    {
      onSuccess(data) {
  ...
  const centerCamera = () => {
    ...
    const locations = data?.orgs.get.locations.map((l) => ({
  ...
  return (
    <View className="flex-1">
      <SafeAreaView className="z-10 absolute bottom-0 w-full">
        ...
        >
          {data?.orgs.get.locations.map((l) => (
            <LocationCard key={l.id} location={l} idOrg={id} />
          ))}
        </Animated.ScrollView>
      </SafeAreaView>
      <MapView
      ...
      >
        {data?.orgs.get.locations.map((l) => (
          <Marker
            key={l.id}
            coordinate={{ latitude: l.locationLat, longitude: l.locationLng }}
          />
        ))}
      </MapView>
    </View>
...
const LocationCard = ({ idOrg, location }: LocationCardProps) => {
  ...
  const uri = `https://maps.googleapis.com/maps/api/streetview?size=800x400&location=${location.locationLat},${location.locationLng}&key=`;
    ...
    Alert.alert("Edit Property", "", [
      {
        ...
        onPress: () => {
          Alert.prompt(
            "Change Name",
            "Enter the new name for this property",
            (name) => {
              updateLocation({
                idOrg,
