import { SafeAreaView, View, Text, Dimensions, Image, AppState } from "react-native";
import MapView, {
    Details,
    Marker as RNMarker,
    PROVIDER_GOOGLE,
...
import { ActionType, Marker, ReservationType, map_style, ReserveStep, RideStep, RideStepType, getDistance } from "@/shared";
import {
    ContextRide,
    ContextRideDispatch,
} from "@/app/authed/state";
...
function getDriveDest(ctxDrive: StateDrive | null) {
    if (!ctxDrive || !ctxDrive.event || (!ctxDrive.dest && ctxDrive.queue.length == 0)) return null;
    // console.log("ctxDrive", ctxDrive);
    const dest = ctxDrive.dest ? ctxDrive.dest : ctxDrive.queue.at(0)!;
    const isDropoff = dest.__typename == "DriverStopEstimationReservation" ? dest.isDropoff : ctxDrive.queue.length != 0;
    if (dest.__typename == "DriverStopEstimationEvent") {
        return {
            type: isDropoff ? MarkerTypes.PICKUP : MarkerTypes.DROPOFF,
            position: {
                latitude: ctxDrive.event.location!.locationLat,
                longitude: ctxDrive.event.location!.locationLng,
            }
        };
    } else {
        return {
            type: isDropoff ? MarkerTypes.DROPOFF : MarkerTypes.PICKUP,
            position: {
                latitude: dest.location.coords.lat,
                longitude: dest.location.coords.lng,
            }
        };
    }
}
...
export default function ScreenMap({
    ...
    const driveStops = ctxDrive?.avaliableReservation?.stops
        .map(s => ({
            type: !ctxDrive.dest ? ctxDrive.avaliableReservation?.isDropoff ? MarkerTypes.DROPOFF : MarkerTypes.PICKUP : MarkerTypes.STOP,
            position: { latitude: s.locationLat, longitude: s.locationLng } }))
        .concat([{ type: !ctxDrive.dest ? ctxDrive.avaliableReservation?.isDropoff ? MarkerTypes.PICKUP : MarkerTypes.DROPOFF : MarkerTypes.STOP, position: latlng(ctxDrive.event?.location!) }]) || [];
    ...
    const onUserMove = (e: UserLocationChangeEvent) => {
        ...
        centerCamera();
        if (!positionForDirections || (positionForDirections && position && getDistance({ lat: positionForDirections.latitude, lng: positionForDirections.longitude }, { lat: position.latitude, lng: position.longitude }) > NEW_DIRECTIONS_THRESHOLD)) {
            if (!isActive) return;
    ...
    return (
        <View className="flex-1">
            ...
            >
                {driveDest && <RNMarker key={driveDest.position.latitude} coordinate={driveDest.position}>
                    <Image source={{ width: 50, height: 100, uri: driveDest.type == MarkerTypes.PICKUP ? "pickup.png" : driveDest.type == MarkerTypes.STOP ? "stop.png" : "dropoff.png" }} />
                </RNMarker>}
                {driveStops && driveStops.map(stop => <RNMarker opacity={1} key={stop.position.latitude} coordinate={stop.position}>
                    <Image source={{ width: 50, height: 100, uri: stop.type == MarkerTypes.PICKUP ? "pickup.png" : stop.type == MarkerTypes.STOP ? "stop.png" : "dropoff.png" }} />
                </RNMarker>)}
                {driveStops.length > 1 && positionForDirections && <MapViewDirections
                    resetOnChange={false}
                ...
                />}
                {driveDest && positionForDirections && <MapViewDirections
                    resetOnChange={false}
                    origin={positionForDirections}
